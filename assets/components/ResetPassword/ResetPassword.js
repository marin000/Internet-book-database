...
function ResetPassword() {
    ...
    const passToken = useParams().id;
    const [userEmail, setUserEmail] = useState("");
    const [error, setError] = useState([]);

    useEffect(() => {
        fetch(`https://localhost:8000/getEmailByPassToken/` + passToken)
          .then(res => res.json())
          .then(
            (result) => {
              setUserEmail(result);
    ...
    return(
        ...
        <Dialog visible={showMessage} onHide={() => setShowMessage(false)} position="top" footer={dialogFooter} showHeader={false} breakpoints={{ '960px': '80vw' }} style={{ width: '30vw' }}>
            <div className="p-d-flex p-ai-center p-dir-col p-pt-6 p-px-3">
                ...
                <h5>Successfully reset!</h5>
                <p style={{ lineHeight: 1.5, textIndent: '1rem' }}>
                    Your password has been successfully reset. You need to log in again.
                </p>
            </div>
        </Dialog>
        <div className="p-d-flex p-jc-center">
            <div className="card">
            <Card>
                <h3 className="p-text-center">Reset password</h3>
